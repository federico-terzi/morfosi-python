...
from morfosi.schema import Action, Change, Add, Delete
from morfosi.tracing.snapshot import snapshot


def assert_add(add: Action, path: List[str], new_value: Any):
...
def assert_change(change: Action, path: List[str], old_value: Any, new_value: Any):
...
def assert_delete(delete: Action, path: List[str], old_value: Any):
